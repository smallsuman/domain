#!/usr/bin/env bash
# Adapted from https://raw.githubusercontent.com/smallsuman/vscode-dev-containers/master/containers/python-3/.devcontainer/library-scripts/python-debian.sh
# Syntax: ./python-debian.sh [Python Version] [Python intall path] [PIPX_HOME] [non-root user] [install tools]

PYTHON_VERSION=${1:-"3.9.0"}
PYTHON_INSTALL_PATH=${2:-"/usr/local/python${PYTHON_VERSION}"}
export PIPX_HOME=${3:-"/usr/local/py-utils"}
USERNAME=${4:-"automatic"}
INSTALL_PYTHON_TOOLS=${5:-"true"}
